...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            var student_uID =
            from s in db.Students orderby s.UId
            select s.UId;

            var professor_uID =
            from p in db.Professors orderby p.UId
            select p.UId;

            var admin_uID =
            from a in db.Administrators orderby a.UId
            select a.UId;
            string ret = "u";
            if (student_uID.Count() + professor_uID.Count() + admin_uID.Count() == 0)
            {
                ret =  "u0000001";
            }
            else {
                int latest_s_uID = Int32.Parse(student_uID.Last().Substring(1));
                int latest_p_uID = Int32.Parse(professor_uID.Last().Substring(1));
                int latest_a_uID = Int32.Parse(admin_uID.Last().Substring(1));

                int uID = new[] { latest_s_uID, latest_p_uID, latest_a_uID }.Max() + 1;
                int num = uID;
                int digit = 0;
                while (num > 0) {
                    num /= 10;
            ...
            else if (role == "Professor")
            {
                ...
                professor.UId = ret;
                professor.FName = firstName;
                professor.LName = lastName;
                professor.Dob = DateOnly.FromDateTime(DOB);
                professor.WorksIn = departmentAbbrev;
